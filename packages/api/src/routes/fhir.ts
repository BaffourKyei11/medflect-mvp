...
fhirRouter.post('/:type', async (req,res,next)=>{ try{ const {type}=req.params; const doc=await createResource(type,req.body); res.status(201).json(doc); }catch(e){ next(e);} });
fhirRouter.put('/:type/:id', async (req,res,next)=>{ try{ const {type,id}=req.params; res.json(await updateResource(type,id,req.body)); }catch(e){ next(e);} });
