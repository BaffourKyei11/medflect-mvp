...
const ajv: any = new (Ajv as any)({ allErrors: true, removeAdditional: 'failing' });
...
const validateResource = (typeParam: string) => (req: any, res: any, next: any) => {
  try {
    const type = (typeParam || '').trim();
    const body = req.body || {};
    if (!body || typeof body !== 'object') return res.status(400).json({ error: 'Invalid JSON body' });
    if (!body.resourceType) return res.status(400).json({ error: 'Missing resourceType' });
    if (body.resourceType !== type) return res.status(400).json({ error: `resourceType must be ${type}` });
