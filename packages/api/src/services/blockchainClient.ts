...
const memoryConsent = new Map<string, Set<string>>();

type AccessLogContract = ethers.Contract | null;
type ConsentTokenContract = ethers.Contract | null;

let provider: ethers.Provider | null = null;
let wallet: ethers.Wallet | null = null;
let accessLog: AccessLogContract = null;
let consentToken: ConsentTokenContract = null;
let initialized = false;
let patientAddrMap: Record<string, string> | null = null;
...
async function loadPatientAddressMap() {
  if (patientAddrMap) return patientAddrMap;
  ...
  for (const p of candidates) {
    try {
      ...
      if (obj && typeof obj === 'object') {
        patientAddrMap = obj as Record<string, string>;
...
function hmacSha256Hex(key: string, payload: string) {
...
async function initIfNeeded() {
  ...
  try {
    ...
    let accessLogAbi: any;
    let consentTokenAbi: any;
    try {
    ...
    console.warn('[Blockchain] Init error; falling back to memory. Reason:', (e as Error).message);
    provider = null; wallet = null; accessLog = null; consentToken = null;
  }
}

async function recordAuditHashInternal(obj: Record<string, any>, consentId = 0, actionType = 'event') {
  await initIfNeeded();
  ...
  try {
    const hash32 = '0x' + hex;
    const tx = await (accessLog as ethers.Contract).logAccess(consentId, hash32, actionType);
    const rec = await tx.wait();
    return { hashHex: hex, txHash: rec?.hash };
  } catch (e) {
    console.warn('[Blockchain] recordAuditHash failed:', (e as Error).message);
    return { hashHex: hex, txHash: undefined };
  }
}
...
export const blockchainClient = {
  ...
  async grantConsent(patientId: string, category: string) {
    const s = memoryConsent.get(patientId) || new Set<string>();
    s.add(category);
    ...
    try {
      ...
      if (consentToken && !getEnvBool('DISABLE_BLOCKCHAIN', true)) {
        ...
        if (addr && ethers.isAddress(addr)) {
          const tx = await (consentToken as ethers.Contract).grantConsentFor(addr, [category], ['treatment']);
          await tx.wait();
      ...
      console.warn('[Blockchain] grantConsentFor failed; continuing in-memory. Reason:', (e as Error).message);
    }
    return true;
  },

  async revokeConsent(patientId: string, category: string) {
    const s = memoryConsent.get(patientId) || new Set<string>();
    s.delete(category);
    memoryConsent.set(patientId, s);
  ...
  async checkConsent(patientId: string, category: string) {
    return memoryConsent.get(patientId)?.has(category) ?? false;
  },

  async recordAuditHash(obj: Record<string, any>, consentId = 0, actionType = 'event') {
